...
class RFMService
{
    ...
    public static function rfm($subQuery, $rfmPrms)
    {
        ...
        $rCount = DB::table($subQuery)->rightJoin('ranks', 'ranks.rank', '=', 'r')
            ->groupBy('rank')->selectRaw('rank as r,count(r)')->orderBy('r', 'desc')->pluck('count(r)'); //購入者をrランクごとにまとめて、rランクについてそれぞれのレベルごとに何人の購入者が存在するかをのデータを取得し、ランクの高い順に並べ、その人数だけを抽出した配列。
        ...
        $fCount = DB::table($subQuery)->rightJoin('ranks', 'ranks.rank', '=', 'f')
            ->groupBy('rank')->selectRaw('rank as r,count(f)')->orderBy('f', 'desc')->pluck('count(f)');//購入者をfランクごとにまとめて、fランクについてそれぞれのレベルごとに何人の購入者が存在するかをのデータを取得し、ランクの高い順に並べ、その人数だけを抽出した配列。



        $mCount = DB::table($subQuery)->rightJoin('ranks', 'ranks.rank', '=', 'm')
            ->groupBy('rank')->selectRaw('rank as r,count(m)')->orderBy('m', 'desc')->pluck('count(m)'); //購入者をmランクごとにまとめて、mランクについてそれぞれのレベルごとに何人の購入者が存在するかをのデータを取得し、ランクの高い順に並べ、その人数だけを抽出した配列。
        ...
        $data = DB::table($subQuery)->rightJoin('ranks', 'ranks.rank', '=', 'r')
            ->groupBy('rank')
