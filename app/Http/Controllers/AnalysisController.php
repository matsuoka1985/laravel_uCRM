...
class AnalysisController extends Controller
{
    public function index()
    {
        ...
        $rCount = DB::table($subQuery)->rightJoin('ranks', 'ranks.rank', '=', 'r')
        ->groupBy('rank')->selectRaw('rank as r,count(r)')->orderBy('r', 'desc')->pluck('count(r)');

        $fCount = DB::table($subQuery)->rightJoin('ranks', 'ranks.rank', '=', 'f')
        ->groupBy('rank')->selectRaw('rank as r,count(f)')->orderBy('f', 'desc')->pluck('count(f)');

        $mCount = DB::table($subQuery)->rightJoin('ranks', 'ranks.rank', '=', 'm')
        ->groupBy('rank')->selectRaw('rank as r,count(m)')->orderBy('m', 'desc')->pluck('count(m)');
    ...
    public function decile()
    {
        ...
        $total = $total[0]->total; //構成費用

        $decile = ceil($count / 10); // 10/1の件数を変数に入れる。
        ...
        //1~10/1をグループ1に、10分の1から10分の2をグループ2に
        DB::statement('set @row_num=0;');
        $subQuery = DB::table($subQuery)
            ->selectRaw("
            row_num,
    ...
    public function rfm(){
        ...
        $total = DB::table($subQuery)->count();
        $rCount = DB::table($subQuery)->rightJoin('ranks','ranks.rank','=','r')
        ->groupBy('rank')->selectRaw('ranks as r,count(r)')->orderBy('r', 'desc')->pluck('count(r)');

        $fCount = DB::table($subQuery)->rightJoin('ranks', 'ranks.rank', '=', 'f')
        ->groupBy('rank')->selectRaw('ranks as r,count(f)')->orderBy('f', 'desc')->pluck('count(f)');

        $mCount = DB::table($subQuery)->rightJoin('ranks', 'ranks.rank', '=', 'm')
        ->groupBy('rank')->selectRaw('ranks as r,count(m)')->orderBy('m', 'desc')->pluck('count(m)');
